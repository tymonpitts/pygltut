...
class Instance(object):
    def calcOffset(self, fElapsedTime):
        raise NotImplementedError

    def constructMatrix(self, fElapsedTime):
        theMat = Matrix()
        point = self.calcOffset(fElapsedTime)
        for index in xrange(4):
            theMat[3, index] = point[index]
        return theMat

class StationaryOffset(Instance):
    def calcOffset(self, fElapsedTime):
        return Point(0, 0, -20)

class OvalOffset(Instance):
    def calcOffset(self, fElapsedTime):
        fLoopDuration = 3.0
        ...
        return Point(
                math.cos(fCurrTimeThroughLoop * fScale) * 4.0,
                math.sin(fCurrTimeThroughLoop * fScale) * 6.0,
                -20.0)

class BottomCircleOffset(Instance):
    def calcOffset(self, fElapsedTime):
        fLoopDuration = 12.0
        fScale = math.pi * 2.0 / fLoopDuration
        ...
        return Point(
                math.cos(fCurrTimeThroughLoop * fScale) * 5.0,
                -3.5,
                math.sin(fCurrTimeThroughLoop * fScale) * 5.0 - 20.0)
